...
/**
 * Create a draft payroll run against a specific Payroll Period. year/month are derived
 * from the period's end_date — they remain a display label and feed the statutory
 * rate/PCB effective-date lookups, but the period's own start_date/end_date (not
 * year/month) is what calculatePayrollRun() actually uses to select attendance data.
 * UNIQUE(payroll_period_id) prevents creating more than one run for the same period.
 */
export function createPayrollRun(
  ...
  try {
    const result = db.prepare(`
      INSERT INTO payroll_runs (payroll_period_id, year, month, status, run_date, created_at, updated_at)
      VALUES (@payroll_period_id, @year, @month, 'draft', @run_date, @created_at, @updated_at)
    `).run({
    ...
    if (msg.includes('UNIQUE') || msg.includes('UNIQUE constraint')) {
      throw new Error(`A payroll run for period "${period.name}" already exists`)
    }
    throw err
  }
}
