...
describe('migration 0021 — commission-only payroll on top of 0020 payroll_period linking', () => {
  it('preserves pre-existing salary_structures and payroll_runs rows through both migrations', () => {
    ...
    // the 0.2.9 incident).
    const structure = db
      .prepare('SELECT rate_type, rate_amount, pcb_category, pcb_children_count FROM salary_structures WHERE id = 1')
    ...
    const item = db.prepare('SELECT salary_structure_id, statutory_base FROM payroll_run_items WHERE payroll_run_id = 1').get() as
      { salary_structure_id: number; statutory_base: number }
    expect(item.salary_structure_id).toBe(1)
    expect(item.statutory_base).toBe(0) // historical rows default to 0 (informational only)
